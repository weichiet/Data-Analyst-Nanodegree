...
`'poi_id.py'`: This script generates the pickle files that reflect the final classification model.

`'my_dataset.pkl'`, `'my_classifier.pkl'`, `'my_feature_list.pkl'`: These three pickle files store the dataset, classifier and list of features that can be used to evaluate the performance of final model by using the `'tester.py'` script.

`'identify_fraud.ipynb'`: This Jupyter notebook provides all the detail steps and analysis in deriving the final classification model. The HTML version of this notebook is also provided.
